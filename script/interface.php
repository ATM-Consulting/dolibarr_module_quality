<?php
	...
	switch($get) {
		case 'quality_definition':
			...
			echo _get_quality_definition_form(GETPOST('fk_object'),GETPOST('type_object'),GETPOST('qty'));
	...
	switch($put){
		...
		case 'quality_definition':
			...
			foreach($TQuality as $code=>$data) {
				...
				$q=new TQuality;
				$q->loadByObjectCode($PDOdb, (int)GETPOST('fk_object'), GETPOST('type_object'), $code);
				$q->set_values($data);
				$q->type_object = GETPOST('type_object');
				$q->fk_object = GETPOST('fk_object');
				$q->code = $code;
				$q->save($PDOdb);
